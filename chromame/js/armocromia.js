 ...
 * @param {number} skinL
 * @param {number} deltaL - estimated |skinL - hairL|
 * @returns {string}
 */
function mapSeasonSimple(undertone, skinL, deltaL) {
...
export async function runArmocromia() {
  ...
  renderArmocromiaResults(skin, skinLab, undertoneLabel, season, seasonData, minDelta, deltaLabel, garmentHex);
...
function renderArmocromiaResults(skin, skinLab, undertoneLabel, season, seasonData, minDelta, deltaLabel, garmentHex) {
  ...
  const el = document.getElementById('armo-results');
  el.innerHTML = `
